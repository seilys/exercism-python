...
def is_criticality_balanced(temperature, neutrons_emitted):
    ...
    """
    return temperature < MIN_TEMP and neutrons_emitted > MAX_NEUTRONS and (temperature * neutrons_emitted) < MAX_OK
...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    """
    status = (voltage * current / theoretical_max_power)*100
    if (status >= 80):
        return 'green'
    if (60 <= status and status < 80):
        return 'orange'
    if (30 <= status and status < 60):
        return 'red'
    if (status < 30):
        return 'black'
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    if status < 0.9 * threshold:
        return 'LOW'
    elif status >= 0.9 * threshold and status <= threshold * 1.1:
        return 'NORMAL'
    else:
        return 'DANGER'
