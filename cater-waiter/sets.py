...
def is_alcoholic(drink_ingredients):
...
def check_drinks(drink_name, drink_ingredients):
    ...
    """
    if is_alcoholic(drink_ingredients):
...
def tag_special_ingredients(dish):
    ...
    """
    specials = set(dish[1]).intersection(SPECIAL_INGREDIENTS)
    if len(specials) > 0:
        return clean_ingredients(dish[0], specials)
...
def singleton_ingredients(dishes, intersection):
    ...
    """
    return set(chain.from_iterable(dishes)).difference(intersection)
